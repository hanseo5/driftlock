...
surface, `browser-evidence.json` with enough evidence for the next Lodestar
...
- Browser evidence is required before final QA when the product has a rendered
  UI surface.
- If build or QA fails, route to `lodestar-eva`.
- If browser/design evidence is required by the product shape, refresh
  `browser-evidence.json` or route to `lodestar-guard`.
