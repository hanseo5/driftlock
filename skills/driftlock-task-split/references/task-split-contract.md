...
Produce `task-graph.json` with enough evidence for the next Driftlock skill to continue without re-interrogating product intent.
...
```powershell
driftlock.py execution-start --task-graph task-graph.json --spec locked-spec.json --run-dir .driftlock/runs/<run-id>
```
...
builder, reviewer, fixer, compound, and amendment roles advance through the
