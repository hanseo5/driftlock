# Driftlock Orchestrator Contract

## Purpose

Run a complete Driftlock delegation from one request, holding every gate, while
...
| --- | --- | --- |
| Discovery | office-hours, brainstorm, grill, intent-brief | Ambiguity that changes product shape is resolved with the user before planning. |
...
| Escalation/Delivery | amend-advisor, compound, handoff | CE cannot jump to review or handoff; it returns through fixer or implementer/builder. Final handoff requires build, review, UX guard, amendment, integration, QA, and proof evidence. |
...
Begin at `driftlock-office-hours`. If a request would change product intent, UX
lock, or spec meaning mid-run, route to `driftlock-amend-advisor`.
