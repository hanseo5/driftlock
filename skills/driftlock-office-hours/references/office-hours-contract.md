# Driftlock Office Hours Contract
...
Use these as reference contracts, not directly exposed skills. Driftlock keeps
...
- Any prior Driftlock run notes relevant to the same product
- `references/upstream-map.json`

## Required Output

Produce `office-hours.md` with enough evidence for the next Driftlock skill to continue without re-interrogating product intent.
...
- recommendation for next Driftlock route
...
- Route visual/product-shape questions to `driftlock-ux-preview` or
  `driftlock-decision-card` instead of burying them in prose.
